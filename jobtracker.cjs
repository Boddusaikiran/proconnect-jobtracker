// CommonJS version of jobtracker.js for projects with "type":"module" in package.json
// Run: node jobtracker.cjs
...
app.get('/', (req, res) => {
   res.send('<html><body><h3>JobTracker CJS Demo - visit /api/jobs</h3></body></html>');
});
...
server.listen(PORT, () => { console.log('JobTracker demo (CJS) running at http://localhost:' + PORT); });
