/**
 * jobtracker.js
 * Single-file Full-stack Starter for "Ultimate JobTracker"
 *
 * Run: node jobtracker.js
 *
 * Notes:
 * - This is a demo prototype. Replace the in-memory stores with a real DB (Mongo/Postgres).
 * - AI endpoints are stubs — you can wire OpenAI or other models where indicated.
 * - File upload persists in-memory as base64 — replace with S3 or disk storage for production.
 *
 * Dependencies:
 *   npm i express socket.io multer jsonwebtoken bcryptjs cors
 */
...
// In-memory "DB" (demo)
const db = {
   users: [], // { id, name, email, passwordHash, profile, resumes: [{id, name, data}] , skills: [], prefs:{} }
   jobs: [],  // { id, title, company, location, remote, description, skills, salaryRange, postedBy, applicants:[] }
   sessions: {}, // token->userId (for demo only)
};
...
// Multer for resume upload (in-memory)
...
app.get('/api/users/me', authMiddleware, (req, res) => {
   ...
   res.json({
      ...
      profile: u.profile,
      skills: u.skills,
      resumes: u.resumes.map(r => ({ id: r.id, name: r.name })),
      prefs: u.prefs
   });
});

app.post('/api/users/me/profile', authMiddleware, (req, res) => {
   const u = req.user;
   u.profile = { ...(u.profile || {}), ...(req.body || {}) };
   res.json({ message: 'Profile updated', profile: u.profile });
});
...
app.post('/api/jobs', authMiddleware, (req, res) => {
   // Basic posting flow (recruiters would have extra checks)
   ...
   db.jobs.push(job);
   io.emit('job:posted', job); // realtime notify
   res.json({ job });
});

app.get('/api/jobs', (req, res) => {
   // Basic search & filters via query params
   const { q, skill, remote } = req.query;
   let results = db.jobs.slice().reverse();
   if (q) {
      const ql = q.toString().toLowerCase();
      results = results.filter(j => (j.title + ' ' + j.company + ' ' + j.description).toLowerCase().includes(ql));
   }
...
app.post('/api/jobs/:id/apply', authMiddleware, (req, res) => {
   ...
   // Record basic application event (expand as needed)
...
// ----------------- AI / SMART STUBS -----------------
/**
 * NOTE: These are stubs. Replace implementations with real AI calls (OpenAI, Azure, Anthropic, local LLMs).
 * If you set OPENAI_API_KEY in env, you can wire calls here.
 */
const OPENAI_KEY = process.env.OPENAI_API_KEY || null;
async function aiRewriteResume(text, roleHint = '') {
   // Very simple demo transformation: append a tagline and "AI-optimized" marker.
   // TODO: call OpenAI / other LLM here
   return `--- AI-Optimized Resume (${roleHint || 'general'}) ---\n\n${text}\n\n[AI SUGGESTIONS]\n- Tailor bullets to metrics\n- Add keywords from job postings\n---`;
}

app.post('/api/ai/rewrite-resume', authMiddleware, async (req, res) => {
   const { resumeId, jobTitle, rawText } = req.body;
   // rawText preferred: the raw resume text; if absent, load uploaded resume (base64) and do minimal extraction
   let text = rawText;
   if (!text && resumeId) {
      const r = (req.user.resumes || []).find(x => x.id === resumeId);
      if (r) {
         // naive: return base64 as decoded text — in production parse PDF/DOCX
         text = Buffer.from(r.data, 'base64').toString('utf8').slice(0, 3000);
      }
   }
   if (!text) return res.status(400).json({ message: 'No resume text provided' });
   const rewritten = await aiRewriteResume(text, jobTitle || '');
   res.json({ rewritten });
});
...
app.post('/api/ai/match-score', authMiddleware, (req, res) => {
   ...
   if (!job) return res.status(404).json({ message: 'Job not found' });
   // naive scoring: based on skill overlap
...
// AI: Fake "career assistant" reply (chat)
app.post('/api/ai/chat', authMiddleware, async (req, res) => {
   const { message } = req.body;
   // TODO: call a real LLM; here we produce a friendly reply
...
io.on('connection', (socket) => {
   // For demo, allow client to identify as a user (not secure in prod)
   socket.on('identify', (userId) => {
      socket.join(userId); // join room for user
   });
   socket.on('ping', (data) => {
      socket.emit('pong', { at: now(), data });
   });
});

// ----------------- SIMPLE FRONTEND (React via CDN) -----------------
app.get('/', (req, res) => {
   // Serve a single-page app that uses React from CDN to keep file self-contained.
...
(function seed() {
   ...
   (async () => {
      ...
      db.users.push(u1, u2);
      db.jobs.push({
         id: uid('job'), title: 'Senior Frontend Engineer', company: 'NovaTech', location: 'Bengaluru, India', remote: true,
         description: 'Work on modern React apps, build performant UIs, mentor juniors.',
         skills: ['JavaScript', 'React', 'TypeScript'], salaryRange: { min: 1200000, max: 2000000 }, postedBy: u1.id, applicants: [], createdAt: now()
      });
      db.jobs.push({
         id: uid('job'), title: 'Machine Learning Engineer', company: 'Aether Labs', location: 'Remote', remote: true,
         description: 'Build models, deploy ML services, collaborate across teams.',
         skills: ['Python', 'ML', 'TensorFlow'], salaryRange: { min: 1500000, max: 2500000 }, postedBy: u2.id, applicants: [], createdAt: now()
      });
...
// ----------------- START SERVER -----------------
server.listen(PORT, () => {
   console.log('JobTracker demo running at http://localhost:' + PORT);
});
